...
int main( int argc , char** argv ) {
  ...
  if ( argc == 3 && !strcmp( "-nt" , argv[1] ) ) {
    ...
    new_C = atof(argv[4]); 
    printf("\n Adjust the chain density to  %f\n" , new_C ) ;
    new_chiAB = atof(argv[6]); 
        printf("\n Adjust the new chiAB  to  %f\n" , new_chiAB ) ;
  ...
  for ( step = 0 ; step < nsteps ; step++ )  {
   ...
   if ( step > sample_wait && step % sample_freq == 0 ) {
    ...
    if ( step % print_freq == 0 || step == nsteps-1 ) {
      ...
      fprintf( otp , "%d %lf %lf %lf %lf %lf %lf %lf\n" , step , Ubond , U_kappa_gg , U_chi_gg, U_kappa_pp,U_chi_pg,Ubond + U_kappa_gg + U_chi_gg +U_kappa_pp+U_chi_pg,Pscalar) ;
      fflush( otp ) ;
      ...
      for ( i=0 ; i<Dim ; i++ )
        for ( j=i+1 ; j<Dim ; j++ )
          fprintf( nbo , "%e " , Rg3*Stress_nb[i][j] ) ;
      ...
      for ( i=0 ; i<Dim ; i++ )
        for ( j=i+1 ; j<Dim ; j++ )
          fprintf( bono, "%e " , -Rg3*Stress_bonds[i][j] ) ;

      fprintf( bono , "\n" ) ; fflush( bono ) ;

     for ( i=0 ; i<Dim ; i++ )
         fprintf( tt_po , "%lf " , Rg3*(rho0)-Rg3*Stress_bonds[i][i] - Rg3*Stress_nb[i][i]) ;
     for ( i=0 ; i<Dim ; i++ )
        for ( j=i+1 ; j<Dim ; j++ )
          fprintf( tt_po, "%e  " , -Rg3*Stress_bonds[i][j] - Rg3*Stress_nb[i][j])  ;
